...
4. The client stores the refresh token securely and attaches the accesstoken to subsequent requests.
### Extensions
...
- **2b.** Account exists but is not yet verified: system rejects loginand offers to resend a verification message.
...
- **Postconditions:** A `Post` exists in the `Published` state, visibleon the public feed and the author's profile.
...
5. The system transitions the post to `Published` and records a publishtimestamp.
